...
void M(double** A, double** B, int n) { // функция для вычисления обратной матрицы
	for (int k = 0; k < n; ++k)
	{
		double div = A[k][k];
...
int main() {
	...
	M(H, H1, 2); // вычисление матрицы H-1
	...
	double l = pow((L(Fx(x0, y0), Fy(x0, y0))), 0.5);
	...
	cout << "Алгоритм сошёлся за " << it << " итераций.\n";
	cout << "Искомая точка минимума X = (" << x01 << "; " << y01 << ")\n";
	cout << "Значение функции в минимуме: F = " << F(x01, y01) << endl;
	cout << "При l = " << l << " множитель сходимости q = 1, при l > " << l << " q < 1." << endl;
	return 0;
}
